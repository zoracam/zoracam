...
- マイクへのアクセス：Live Photo モードが有効な場合に環境音を収録するためのものです。すべての音声は端末上でローカル処理され、ユーザーが保存を選択した Live Photo にのみ含まれます。アプリは音声をいかなるサーバーにもアップロード・送信しません。**端末内でのみ処理され、同期やバックアップは行われません**。
...
### 2.3 音声データの処理
- Live Photo モードが有効な場合、アプリは写真と同期した短い音声クリップを収録します。音声の処理はすべて端末上で行われ、インターネット接続は不要で、いかなるサーバーにも送信されません。音声は収録中のみ一時メモリバッファに存在し、Live Photo を保存した場合はメディアファイルの一部として端末に保存されます。

## 3. データ保存と安全性
- **保存場所**：すべての写真およびデータ処理は端末内で行われ、Live Photo に関連する音声はメディアファイル内で端末にローカル保存されます。アプリはクラウドバックアップや同期を行いません。システム設定で iCloud 写真を有効にしている場合、Apple のポリシーに従ってメディアが同期されることがありますが、これは当アプリの管理外です。**クラウドストレージは使用しません**。  
- **安全対策**：  
