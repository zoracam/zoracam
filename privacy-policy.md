...
- 麦克风访问：用于在启用 Live Photo 模式时采集环境音频。所有音频仅在您的设备本地处理，并仅包含于您选择保存的 Live Photo 中。应用不会将音频上传或传输到任何服务器。
...
### 2.3 音频数据处理
- 当 Live Photo 模式处于启用状态时，应用会捕获与照片同步的短音频片段。所有音频处理完全在您的设备上进行，不需要互联网连接，也不会传输至任何服务器。音频仅在采集期间存在于临时内存缓冲区中；若您保存该 Live Photo，音频将作为媒体文件的一部分存储在您的设备上。

## 3. 数据存储与安全
- ​**存储位置**：所有照片和数据处理均在您的设备本地完成。与 Live Photo 关联的音频以媒体文件形式存储在您的设备本地。应用不进行任何云备份或同步。若您在系统层面启用 iCloud 照片，Apple 可能会根据其政策同步您的媒体；该行为超出本应用的控制范围。​**不涉及云端存储**。
- ​**安全措施**：  
  - 采用沙盒机制、加密技术保护数据。  
  - 处理完成后立即释放内存，​**不留存任何副本**。
